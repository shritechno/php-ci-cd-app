        ...
        stage('Deploy to EC2') {
            steps {
                sshagent (credentials: ['ec2-key']) {
                    sh '''
                    rsync -avz --exclude='.git' -e "ssh -o StrictHostKeyChecking=no" ./ ubuntu@15.206.89.44:/home/ubuntu/deploy-temp/

                    ssh -o StrictHostKeyChecking=no ubuntu@15.206.89.44 bash -c '
