pipeline {
    ...
    steps {
        sshagent (credentials: ['ec2-key']) {
            sh '''
            rsync -avz --exclude='.git' -e "ssh -o StrictHostKeyChecking=no" ./ ubuntu@3.110.124.247:/home/ubuntu/deploy-temp/

            ssh -o StrictHostKeyChecking=no ubuntu@3.110.124.247 bash -c '
